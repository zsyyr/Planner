	...
	g_object_class_install_property (object_class,
        ...
        g_object_class_install_property (object_class,
                                                ...
                                                g_param_spec_boolean ("exclusive",
                                                                    "Exclusive",
                                                                    "The resource could be shared or not",
                                                                    TRUE,//default_value
                                                                    G_PARAM_READWRITE));
        g_object_class_install_property (object_class,
                                                        PROP_HASIT,
                                                        g_param_spec_boolean ("hasit",
                                                                            "hasit",
                                                                            "has it certification",
                                                                            TRUE,//default_value
                                                                            G_PARAM_READWRITE));
        g_object_class_install_property (object_class,
                                                        PROP_HASC,
                                                        g_param_spec_boolean ("hasc",
                                                                            "hasc",
                                                                            "has certification",
                                                                            TRUE,//default_value
                                                                            G_PARAM_READWRITE));
		       ...
		       GParamSpec   *pspec)
{
	...
	case PROP_EXCLUSIVE:
		i_val = g_value_get_boolean(value);

			if (priv->exclusive != i_val) {
				priv->exclusive = i_val;
				changed = TRUE;
			}
		break;
	case PROP_HASIT:
		       ...
		       GParamSpec *pspec)
{
	...
	case PROP_COST:
		...
		break;
	case PROP_HASIT:
			g_value_set_float (value, priv->hasit);
			break;
	case PROP_HASC:
			g_value_set_float (value, priv->hasc);
			break;
	default:
...
//zms
gboolean
mrp_resource_get_hasit(MrpResource *resource)
{
	g_return_val_if_fail (MRP_IS_RESOURCE (resource), NULL);

	return resource->priv->hasit;
}
gboolean
mrp_resource_get_hasc(MrpResource *resource)
{
	g_return_val_if_fail (MRP_IS_RESOURCE (resource), NULL);

	return resource->priv->hasc;
}
void mrp_resource_set_hasit (MrpResource *resource, gboolean hasit)
{
	g_return_if_fail (MRP_IS_RESOURCE (resource));

	mrp_object_set (MRP_OBJECT (resource), "hasit", hasit, NULL);
}
void mrp_resource_set_hasc (MrpResource *resource, gboolean hasc)
{
	g_return_if_fail (MRP_IS_RESOURCE (resource));

	mrp_object_set (MRP_OBJECT (resource), "hasc", hasc, NULL);
}

const gchar *
mrp_resource_get_note (MrpResource *resource)
{
	g_return_val_if_fail (MRP_IS_RESOURCE (resource), NULL);

	return resource->priv->note;
}
