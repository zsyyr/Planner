...
task_class_init (MrpTaskClass *klass)
{
	...
	g_object_class_install_property (
			object_class,
			PROP_VIRTURALTYPE,
			g_param_spec_enum ("virtualtype",
					   "VirtualType",
					   "Task virtual type",
					   MRP_TYPE_TASK_VIRTUAL_TYPE,
					   MRP_TASK_TYPE_ACTUAL,
					   G_PARAM_READWRITE));
		   ...
		   GParamSpec   *pspec)
{
	...
	case PROP_VIRTURALTYPE:
		virtualtype = g_value_get_enum (value);
		if (virtualtype != priv->virtualtype) {
			priv->virtualtype = virtualtype;
		}
		break;
		   ...
		   GParamSpec *pspec)
{
	...
	case PROP_TYPE:
		...
		break;
	case PROP_VIRTURALTYPE:
			g_value_set_enum (value, priv->virtualtype);
			break;
	case PROP_SCHED:
	...
	case PROP_PRIORITY:
		...
		break;
	case PROP_PROBABILITY:
		g_value_set_int (value, priv->probability);
		break;
	case PROP_ID:
			g_value_set_int (value, priv->id);
			break;
	case PROP_NEEDIT:
			g_value_set_boolean (value, priv->needit);
			break;
	case PROP_NEEDC:
...
task_removed (MrpObject *object)
{
	...
	task_remove_group_assignments (task);
...
static void
task_group_assignment_removed_cb (MrpGroupAssignment *assignment, MrpTask *task)
{
	MrpTaskPriv *priv;
	...
	priv->group_assignments = g_list_remove (priv->group_assignments, assignment);
...
static void
task_remove_group_assignments (MrpTask *task)
{
	MrpTaskPriv   *priv;
	GList         *copy, *l;
	MrpGroupAssignment *group_assignment;

	g_return_if_fail (MRP_IS_TASK (task));

	priv = task->priv;

	copy = g_list_copy (priv->group_assignments);

	for (l = copy; l; l = l->next) {
		group_assignment = l->data;

		g_signal_handlers_disconnect_by_func (MRP_ASSIGNMENT (l->data),
						      task_group_assignment_removed_cb,
						      task);
		g_object_unref (group_assignment);
		mrp_object_removed (MRP_OBJECT (group_assignment));
	}

	g_list_free (priv->group_assignments);
	g_list_free (copy);
 ...
 **/
void
imrp_task_add_group_assignment (MrpTask *task, MrpGroupAssignment *assignment)
{
	MrpTaskPriv *priv;
	...
	priv->group_assignments = g_list_prepend (priv->group_assignments,
					    g_object_ref (assignment));

	g_signal_connect (assignment,
			  "removed",
			  G_CALLBACK (task_group_assignment_removed_cb),
			  task);
...
/**
 * mrp_task_get_group_assignment:
 * @task: an #MrpTask
 * @group: an #MrpGroup
 *
 * retrieves the #MrpGroupAssignment associated with @task and @group if the
 * group is assigned to @task, or %NULL if there is no such assignment.
 *
 * Return value: The assignment if it exists, otherwise %NULL.
 **/
MrpGroupAssignment *
mrp_task_get_group_assignment (MrpTask *task, MrpGroup *group)
{
	GList *l;
	...
	for (l = task->priv->group_assignments; l; l = l->next) {
		MrpGroupAssignment *assignment = l->data;

		if (mrp_group_assignment_get_group (assignment) == group) {
			return assignment;
		}
	}
 ...
 */
void
mrp_task_set_probability (MrpTask *task, gint probability)
{
	g_return_if_fail (MRP_IS_TASK (task));

	task->priv->probability = probability;
}
//
...
void
mrp_task_set_task_type (MrpTask *task,MrpTaskType type)
{
	g_return_if_fail (MRP_IS_TASK (task));
...
void
mrp_task_set_task_virtual_type (MrpTask *task,MrpTaskVirtualType type)
{
	g_return_if_fail (MRP_IS_TASK (task));

	task->priv->virtualtype = type;
}
...
MrpTaskVirtualType
mrp_task_get_task_virtual_type (MrpTask *task)
{
	g_return_val_if_fail (MRP_IS_TASK (task), MRP_TASK_TYPE_ACTUAL);

	return task->priv->virtualtype;
}
 ...
 */
gint
mrp_task_get_probability (MrpTask *task)
{
	g_return_val_if_fail (MRP_IS_TASK (task), 0);

	return task->priv->probability;
}

void
mrp_task_set_id (MrpTask *task, gint id)
{
	g_return_if_fail (MRP_IS_TASK (task));
...
void
imrp_task_set_needit (MrpTask *task, gboolean needit)
{
	g_return_if_fail (MRP_IS_TASK (task));

	task->priv->needit = needit;
}

gboolean
imrp_task_get_needit (MrpTask *task)
{
	g_return_val_if_fail (MRP_IS_TASK (task), FALSE);

	return task->priv->needit;
}
void
imrp_task_set_needc (MrpTask *task, gboolean needc)
{
	g_return_if_fail (MRP_IS_TASK (task));

	task->priv->needc = needc;
}

gboolean
imrp_task_get_needc (MrpTask *task)
{
	g_return_val_if_fail (MRP_IS_TASK (task), FALSE);

	return task->priv->needc;
}
