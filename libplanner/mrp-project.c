...
extern gint PROJECTNUMBER;

MrpProject *
renewProject(MrpProject *oldproject)
{
	MrpProjectPriv  *priv;
	MrpApplication *app;
	...
	PROJECTNUMBER++;
	app  = oldproject->priv->app;

	newproject = mrp_project_new (app);
	priv = newproject->priv;
	priv->app= app;
	priv->uri = mrp_project_get_uri(oldproject);
	priv->resources = g_list_copy(mrp_project_get_resources(oldproject));
	priv->groups = g_list_copy(mrp_project_get_groups(oldproject));
	priv->project_start = mrp_project_get_project_start(oldproject);
	priv->name = g_strdup_printf("%d",PROJECTNUMBER);
	priv->needs_saving = oldproject->priv->needs_saving;
	priv->empty = oldproject->priv->empty;
	priv->default_group = oldproject->priv->default_group;
	priv->property_pool = oldproject->priv->property_pool;
	priv->root_calendar = oldproject->priv->root_calendar;
	priv->calendar = oldproject->priv->calendar;
	priv->day_types = oldproject->priv->day_types;
	priv->phases= oldproject->priv->phases;
	priv->phase = oldproject->priv->phase;


	priv->primary_storage = NULL;

	priv->task_manager = imrp_project_get_task_manager(oldproject);
