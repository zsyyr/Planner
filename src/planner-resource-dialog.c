...
static void 		resource_dialog_exclusive_toggled_changed_cb		 (GtkWidget		*w,
								DialogData    *data);
static void			 resource_dialog_notify_exclusive_cb        (MrpResource *resource,
													GParamSpec  *pspec,
													GtkWidget   *dialog);

static void          resource_dialog_name_changed_cb             (GtkWidget     *w,
								  DialogData    *data);
static void          resource_dialog_notify_name_cb              (MrpResource   *resource,
								  GParamSpec    *pspec,
								  GtkWidget     *dialog);
...
static void          resource_dialog_short_name_changed_cb       (GtkWidget     *w,
								  DialogData    *data);
static void          resource_dialog_notify_short_name_cb        (MrpResource   *resource,
								  GParamSpec    *pspec,
								  GtkWidget     *dialog);
...
static void          resource_dialog_type_changed_cb             (GtkWidget     *w,
								  DialogData    *data);
static void          resource_dialog_notify_type_cb              (MrpResource   *resource,
								  GParamSpec    *pspec,
								  GtkWidget     *dialog);
...
static void          resource_dialog_email_changed_cb            (GtkWidget     *w,
								  DialogData    *data);
static void          resource_dialog_notify_email_cb             (MrpResource   *resource,
								  GParamSpec    *pspec,
								  GtkWidget     *dialog);
...
static gboolean      resource_dialog_email_focus_out_cb          (GtkWidget     *w,
								  GdkEventFocus *event,
								  DialogData    *data);
static void          resource_dialog_notify_cost_cb              (MrpResource   *resource,
								  GParamSpec    *pspec,
								  GtkWidget     *dialog);
...
static gboolean      resource_dialog_resource_cost_focus_out_cb  (GtkWidget     *w,
								  GdkEventFocus *event,
								  DialogData    *data);
static void          resource_dialog_notify_calendar_cb          (MrpResource   *resource,
								  GParamSpec    *pspec,
								  GtkWidget     *dialog);
static void          resource_dialog_edit_calendar_clicked_cb    (GtkWidget     *button,
								  DialogData    *data);
static void          resource_dialog_notify_note_cb              (MrpResource   *resource,
								  GParamSpec    *pspec,
								  GtkWidget     *dialog);
...
//zms add for resource->exclusive cb
static void
resource_dialog_exclusive_toggled_changed_cb		 (GtkWidget		*w,
											DialogData    *data)
{
	gboolean  exclusive;
	gboolean  exclusive1;
	gchar     *e;
		...
		exclusive = gtk_toggle_button_get_active (w);

		g_signal_handlers_block_by_func (data->resource,
						 resource_dialog_notify_exclusive_cb,
						 data->dialog);

		g_object_set (G_OBJECT(data->resource), "exclusive", exclusive, NULL);


		g_signal_handlers_unblock_by_func (data->resource,
						   resource_dialog_notify_exclusive_cb,
						   data->dialog);
		g_object_get (G_OBJECT(data->resource), "exclusive", &exclusive1, NULL);
		if(exclusive1)
				e = "Yes";
			else
				e = "No";
		gtk_button_set_label(w,e);

			//resource_dialog_update_title (data);
}
static void
resource_dialog_notify_exclusive_cb        (MrpResource *resource,
									        GParamSpec  *pspec,
									        GtkWidget   *dialog)
{
DialogData *data;
...
g_object_get (data->resource, "exclusive", &exclusive, NULL);

g_signal_handlers_block_by_func (data->exclusive_taggle,
			 resource_dialog_exclusive_toggled_changed_cb,
			 dialog);

gtk_toggle_button_set_active (data->exclusive_taggle, exclusive);

//resource_dialog_update_title (data);

g_signal_handlers_unblock_by_func (data->exclusive_taggle,
			   resource_dialog_exclusive_toggled_changed_cb,
			   dialog);
...
resource_dialog_connect_to_resource (DialogData *data)
{
	...
	g_signal_connect_object (resource,
					 "notify::exclusive",
					 G_CALLBACK (resource_dialog_notify_exclusive_cb),
					 data->dialog,
					 0);

	g_signal_connect_object (resource,
				 "notify::name",
				 G_CALLBACK (resource_dialog_notify_name_cb),
				 data->dialog,
				 0);

	g_signal_connect_object (resource,
				 "notify::short-name",
				 G_CALLBACK (resource_dialog_notify_short_name_cb),
				 data->dialog,
				 0);

	g_signal_connect_object (resource,
				 "notify::type",
				 G_CALLBACK (resource_dialog_notify_type_cb),
				 data->dialog,
				 0);

	g_signal_connect_object (resource,
				 "notify::group",
				 G_CALLBACK (resource_dialog_notify_group_cb),
				 data->dialog,
				 0);
	...
	g_signal_connect_object (resource,
				 "notify::cost",
				 G_CALLBACK (resource_dialog_notify_cost_cb),
				 data->dialog,
				 0);

	g_signal_connect_object (resource,
				 "notify::calendar",
				 G_CALLBACK (resource_dialog_notify_calendar_cb),
				 data->dialog,
				 0);

	g_signal_connect_object (resource,
				 "notify::note",
				 G_CALLBACK (resource_dialog_notify_note_cb),
				 data->dialog,
				 0);
}
				 ...
				 DialogData *data)
{
	...
	g_signal_handlers_block_by_func (data->resource,
					 resource_dialog_notify_name_cb,
					 data->dialog);
				 ...
				 DialogData *data)
{
	...
	g_signal_handlers_block_by_func (data->resource,
					 resource_dialog_notify_note_cb,
					 data->dialog);
			     ...
			     MrpResource  *resource)
{
	...
	data->exclusive_taggle = glade_xml_get_widget (glade,"togglebutton1");
	...
	gtk_toggle_button_set_active(data->exclusive_taggle,exclusive);
	g_signal_connect (data->exclusive_taggle,
			"toggled",
			G_CALLBACK (resource_dialog_exclusive_toggled_changed_cb),
			data);
