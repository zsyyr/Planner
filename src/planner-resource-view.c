...
static void           resource_view_cell_group_edited        (PlannerCellRendererList *cell,
							      ...
							      gchar                   *new_text,
							      gpointer                 user_data);
static void           resource_view_cell_exclusive_edited   (PlannerCellRendererList *cell,
								  gchar                   *path_string,
								  gchar                   *new_text,
								  gpointer                 user_data);
...
static void           resource_view_email_data_func          (GtkTreeViewColumn       *tree_column,
							      ...
							      GtkTreeIter             *iter,
							      gpointer                 data);
static void 		 resource_view_exclusive_data_func		 (GtkTreeViewColumn       *tree_column,
	      	  	  	  	  	  	  GtkCellRenderer         *cell,
	      	  	  	  	  	  	  GtkTreeModel            *tree_model,
	      	  	  	  	  	  	  GtkTreeIter             *iter,
	      	  	  	  	  	  	  gpointer                 data);
...
resource_view_get_widget (PlannerView *view)
{
	...
	groups = gtk_tree_view_get_columns(tree_view);
...
resource_view_setup_tree_view (PlannerView *view)
{
	...
	col = gtk_tree_view_column_new_with_attributes (_("邮箱"),
							cell, NULL);
	gtk_tree_view_column_set_resizable (col, TRUE);
	gtk_tree_view_column_set_min_width (col, 150);
	...
	col = gtk_tree_view_column_new_with_attributes (_("开销"),cell, NULL);
	gtk_tree_view_column_set_resizable (col, TRUE);
	gtk_tree_view_column_set_min_width (col, 150);
	...
	/*Exclusive*/
		...
		g_object_set (cell,
			...
			col = gtk_tree_view_column_new_with_attributes (_("独占"),
									cell, NULL);
			gtk_tree_view_column_set_resizable (col, TRUE);
			gtk_tree_view_column_set_min_width (col, 150);

			gtk_tree_view_column_set_cell_data_func (col, cell,
								 resource_view_exclusive_data_func,
								 NULL, NULL);
			g_object_set_data (G_OBJECT (col),
					   "data-func", resource_view_exclusive_data_func);
			g_object_set_data (G_OBJECT (col), "id", "exclusive");
			...
			g_signal_connect (cell,
					  "toggled",
					  G_CALLBACK (resource_view_cell_exclusive_edited),
					  view);
...
static void
resource_view_exclusive_data_func		 (GtkTreeViewColumn       *tree_column,
	      	  	  	  	  	  	  GtkCellRenderer         *cell,
	      	  	  	  	  	  	  GtkTreeModel            *tree_model,
	      	  	  	  	  	  	  GtkTreeIter             *iter,
	      	  	  	  	  	  	  gpointer                 data)
...
static void
resource_view_cell_exclusive_edited   (PlannerCellRendererList *cell,
								  gchar                   *path_string,
								  gchar                   *new_text,
								  gpointer                 user_data)
{}
...
resource_view_load_columns (PlannerResourceView *view)
{
	...
	if (i == 0) {
		for (l = columns; l; l = l->next) {
			column = l->data;
			if (g_object_get_data (G_OBJECT (column), "exclusive")) {
