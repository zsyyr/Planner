...
typedef struct {
	...
	GtkWidget     *virtual_checkbutton;
	GtkWidget		*prob_spinbutton;
...
task_cmd_edit_type_do (PlannerCmd *cmd_base)
{
	...
	mrp_task_set_task_type (cmd->task, cmd->type);
...
static gboolean
task_cmd_group_assign_add_do (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;

	cmd = (TaskCmdEditGroupAssignment *) cmd_base;

	/* FIXME: better than returning void it could return the assignment */
	mrp_group_assign (cmd->group, cmd->task, cmd->units);

	return TRUE;
}

static void
task_cmd_group_assign_add_undo (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;
	MrpGroupAssignment         *assignment;
	...
	assignment = mrp_task_get_group_assignment (cmd->task, cmd->group);

	mrp_object_removed (MRP_OBJECT (assignment));
}

static void
task_cmd_group_assign_add_free (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;
...
static PlannerCmd *
task_cmd_group_assign_add (PlannerWindow *main_window,
		     MrpTask       *task,
		     MrpGroup   *group,
		     guint          units)
{
	...
	assignment = mrp_task_get_group_assignment (task, group);
	if (assignment) {
		old_units = mrp_group_assignment_get_units (assignment);
	...
	cmd_base = planner_cmd_new (TaskCmdEditGroupAssignment,
				    _("Assign group to task"),
				    task_cmd_group_assign_add_do,
				    task_cmd_group_assign_add_undo,
				    task_cmd_group_assign_add_free);
...
static gboolean
task_cmd_group_assign_remove_do (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;
	MrpGroupAssignment         *assignment;
...
static void
task_cmd_group_assign_remove_undo (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;
...
static void
task_cmd_group_assign_remove_free (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;
...
static PlannerCmd *
task_cmd_group_assign_remove (PlannerWindow *main_window,
			MrpGroupAssignment *assignment)
{
	PlannerCmd             *cmd_base;
	TaskCmdEditGroupAssignment  *cmd;
	...
	cmd_base = planner_cmd_new (TaskCmdEditGroupAssignment,
				    _("Unassign group from task"),
				    task_cmd_group_assign_remove_do,
				    task_cmd_group_assign_remove_undo,
				    task_cmd_group_assign_remove_free);

	cmd = (TaskCmdEditGroupAssignment *) cmd_base;
	cmd->task = g_object_ref (mrp_group_assignment_get_task (assignment));
	cmd->group = g_object_ref (mrp_group_assignment_get_group (assignment));
	cmd->units = mrp_group_assignment_get_units (assignment);
...
static gboolean
task_cmd_group_assign_units_do (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;
...
static void
task_cmd_group_assign_units_undo (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;
...
static void
task_cmd_group_assign_units_free (PlannerCmd *cmd_base)
{
	TaskCmdEditGroupAssignment *cmd;
...
static PlannerCmd *
task_cmd_group_assign_units (PlannerWindow *main_window,
		       MrpGroupAssignment *assignment,
		       guint          units)
{
	PlannerCmd             *cmd_base;
	...
	cmd_base = planner_cmd_new (TaskCmdEditGroupAssignment,
				    _("Change resource units in task"),
				    task_cmd_group_assign_units_do,
				    task_cmd_group_assign_units_undo,
				    task_cmd_group_assign_units_free);
	...
	cmd->assignment = assignment;
	cmd->units = units;
	cmd->old_units = mrp_group_assignment_get_units (assignment);
...
//zms
static void
task_dialog_virtual_toggled_cb (GtkWidget *w, DialogData *data)
{
	MrpTaskVirtualType virtualtype;

	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (data->virtual_checkbutton))) {
		virtualtype = MRP_TASK_TYPE_VIRTUAL;
	} else {
		virtualtype = MRP_TASK_TYPE_ACTUAL;
	}
	mrp_task_set_task_virtual_type(data->task,virtualtype);
}
...
static void
task_dialog_task_probability_changed_cb (MrpTask    *task,
				      GParamSpec *pspec,
				      GtkWidget  *dialog)
{
	DialogData *data;
	...
	probability = mrp_task_get_probability (task);

	g_signal_handlers_block_by_func (data->prob_spinbutton,
					 task_dialog_complete_changed_cb,
					 data);

	gtk_spin_button_set_value (GTK_SPIN_BUTTON (data->prob_spinbutton),
			probability);

	g_signal_handlers_unblock_by_func (data->prob_spinbutton,
					   task_dialog_complete_changed_cb,
					   data);
}

static void
task_dialog_probability_changed_cb (GtkWidget  *w,
				 DialogData *data)
{
	gint probability;

	probability = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (w));

	g_signal_handlers_block_by_func (data->task,
					 task_dialog_task_probability_changed_cb,
					 data->dialog);
	mrp_task_set_probability(data->task,probability);
	//g_object_set (data->task, "probability", probability, NULL);
...
static gboolean
task_dialog_probability_focus_out_cb (GtkWidget     *w,
				   GdkEventFocus *event,
				   DialogData    *data)
{
	guint         current_probability;
	guint         focus_in_probability;
	GValue        value = { 0 };

	g_assert (MRP_IS_TASK (data->task));

	focus_in_probability = GPOINTER_TO_INT (g_object_get_data
					     (G_OBJECT (data->task), "probability"));

	gtk_spin_button_update (GTK_SPIN_BUTTON (w));
	current_probability = gtk_spin_button_get_value (GTK_SPIN_BUTTON (w));

	if (focus_in_probability == current_probability) {
		return FALSE;
	}
	mrp_task_set_probability(data->task,current_probability);
	//g_object_set (data->task, "probability", current_probability, NULL);

	g_value_init (&value, G_TYPE_UINT);
	g_value_set_uint (&value, focus_in_probability);

//	task_cmd_edit_property_focus (data->main_window,
//				      data->task, "probability", &value);

	return FALSE;
}

static gboolean
task_dialog_probability_focus_in_cb (GtkWidget     *w,
				  GdkEventFocus *event,
				  DialogData    *data)
{
	guint probability;

	probability = gtk_spin_button_get_value (GTK_SPIN_BUTTON (w));

	mrp_task_set_probability(data->task,probability);
	//g_object_set_data (G_OBJECT (data->task),
		//	   "probability", GINT_TO_POINTER (probability));
...
{//todo:aaaa
	...
	resource_object = MRP_OBJECT(((GList *)iter.user_data)->data);
	if(MRP_IS_RESOURCE(resource_object)) {
		MrpResource   *resource;
		MrpAssignment *assignment;
		resource = MRP_RESOURCE(resource_object);
		...
		MrpGroup   *group;
		MrpGroupAssignment *assignment;
		group = MRP_GROUP(resource_object);
		assignment = mrp_task_get_group_assignment (data->task, group);
		if (assignment) {
			value = atoi (new_text);
			if (value != mrp_group_assignment_get_units (assignment)) {
				task_cmd_group_assign_units (data->main_window, assignment, atoi (new_text));
			}
		}
	}
}
			   ...
			   GladeXML   *glade)
{
	...
	g_signal_connect (data->name_entry,
			  ...
			  data);
	//zms
	data->virtual_checkbutton = glade_xml_get_widget (glade, "virtual_checkbutton");
	//g_object_get (data->task, "type", &type, NULL);
	mrp_task_get_task_virtual_type(data->task);
	if (virtualtype == MRP_TASK_TYPE_VIRTUAL) {
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (data->milestone_checkbutton), TRUE);
	}
	g_signal_connect (data->virtual_checkbutton,
			  "toggled",
	...
	data->prob_spinbutton = glade_xml_get_widget (glade, "prob_spinbutton");
	g_object_get (data->task, "probability", &int_value, NULL);
	gtk_spin_button_set_value (GTK_SPIN_BUTTON (data->prob_spinbutton), int_value);
	g_signal_connect (data->prob_spinbutton,
				  "value_changed",
				  G_CALLBACK (task_dialog_probability_changed_cb),
				  data);
		g_signal_connect (data->prob_spinbutton,
				  "focus_in_event",
				  G_CALLBACK (task_dialog_probability_focus_in_cb),
				  data);
		g_signal_connect (data->prob_spinbutton,
				  "focus_out_event",
				  G_CALLBACK (task_dialog_probability_focus_out_cb),
				  data);
				   ...
				   DialogData          *data)
{
	...
	if(MRP_IS_RESOURCE(resource_object)) {
		...
		group = MRP_GROUP(resource_object);
		if (!active) {
			task_cmd_group_assign_add (data->main_window, data->task, group, 100);
		} else {
			MrpGroupAssignment *assignment;
			...
			if (assignment) {
				task_cmd_group_assign_remove (data->main_window, assignment);
			}
		}
	}
}
...
task_dialog_connect_to_task (DialogData *data)
{
	...
	g_signal_connect_object (data->task,
					 "notify::probability",
					 G_CALLBACK (task_dialog_task_probability_changed_cb),
					 data->dialog,
					 0);
